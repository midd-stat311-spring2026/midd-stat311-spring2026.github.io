...
rownames(tab) <- rownames(tab) <- c("\\(Pr(\\sum X_i = y \\mid \\theta_0)\\) ", "\\(\\Lambda(\\mathbf{x})\\)") 
...
-   $\text{Pr}(\Lambda(\mathbf{x}) \leq `r round(Lambda_sort[1], 4)` | \theta = 0.6) = \text{Pr}(\sum X_i = `r y_sort[1]` | \theta = 0.6) = `r round(prob_y_sort[1], 3)` \leq 0.08$ 
-   $\text{Pr}(\Lambda(\mathbf{x}) \leq `r round(Lambda_sort[2], 4)` | \theta = 0.6) = \text{Pr}(\sum X_i = \{`r y_sort[1]`, `r y_sort[2]`\} | \theta = 0.6) = `r round(prob_y_sort[1], 4)` + `r round(prob_y_sort[2], 4)` = `r round(sum(prob_y_sort[1:2]), 3)` \leq 0.08$ 
-   $\text{Pr}(\Lambda(\mathbf{x}) \leq `r round(Lambda_sort[3], 4)` | \theta = 0.6) = \text{Pr}(\sum X_i = \{`r y_sort[1]`, `r y_sort[2]`,`r y_sort[3]` \} | \theta = 0.6) = `r round(prob_y_sort[1], 4)` + `r round(prob_y_sort[2], 4)`+ `r round(prob_y_sort[3], 4)` = `r round(sum(prob_y_sort[1:3]), 3)` > 0.08$
