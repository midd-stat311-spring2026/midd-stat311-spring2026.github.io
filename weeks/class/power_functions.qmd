...
data.frame(theta = theta_vals, power = delta1) |>
  ...
  ylab(expression(pi(theta  ~ "|" ~ delta[1]))) +
...
Consider the other testing procedure $\delta_{2}$: $r(\mathbf{X}) = |\sum_{i=1}^{10} X_{i} - 5| \text{ with rejection region } R = \{r(\mathbf{X}) \geq 4 \}$.

So the power function is $\pi(\theta | \delta_{2}) = P( \sum_{i=1}^{10} X_{i} \in \{0, 1, 9, 10\}|\theta)$.
...
data.frame(theta = theta_vals, delta1, delta2) |>
  ...
  xlab(expression(theta)) +
    ylab(expression(pi(theta  ~ "|" ~ delta)))+
  theme(text = element_text(size = 14))
