...
-   $\text{Pr}(\Lambda(\mathbf{x}) \leq `r round(Lambda_sort[1], 4)` | \theta = 0.6) = \text{Pr}(\sum X_i = \qquad \qquad | \theta = 0.6) =  \qquad$ 
...
In particular, the test that rejects $H_{0}$ when $k = `r round(Lambda_sort[2], 4)`$ has size:

The test that rejects $H_{0}$ when $k = `r round(Lambda_sort[1], 4)`$ has size: 
